...
def get_parser():
    ...
    parser.add_argument('--data_path', default='str', help='data path')
...
class Processor():
    ...
    def eval(self, epoch, save_score=False, loader_name=['test'], wrong_file=None, result_file=None):
        if wrong_file is not None:
            ...
            f_w.write("index,pred,gt,chosen_confidence\n")
        if result_file is not None:
            f_r = open(result_file, 'w')
            f_r.write("index,pred,gt,chosen_confidence\n")
        self.model.eval()
        ...
        for ln in loader_name:
            ...
            for batch_idx, (data, index_t, label, index) in enumerate(process):
                ...
                if wrong_file is not None or result_file is not None:
                    # Confidence in the chosen output class
                    chosen_confidence = F.softmax(output, dim=1)[torch.arange(output.size(0)),predict_label]
                    predict = list(predict_label.cpu().numpy())
                    gt = list(label.data.cpu().numpy())

                    for i, pred_i in enumerate(predict):
                        if result_file is not None:
                            f_r.write(f"{index[i].item()},{pred_i},{gt[i]},{chosen_confidence[i]}\n")
                        if pred_i != gt[i] and wrong_file is not None:
                            f_w.write(f"{index[i].item()},{pred_i},{gt[i]},{chosen_confidence[i]}\n")
    ...
    def start(self):
        if self.arg.phase == 'train':
            ...
            for epoch in range(self.arg.start_epoch, self.arg.num_epoch):
                ...
                self.eval(epoch, save_score=True, loader_name=['val'])

            # test the best model
            weights_path = glob.glob(os.path.join(self.arg.work_dir, 'runs-' + str(self.best_acc_epoch) + '*'))[0]
            self.print_log(f"Best accuracy epoch: {self.best_acc_epoch}")
            self.print_log(f"Best model stored at {weights_path}")
            ...
            rf = os.path.join(self.arg.work_dir, 'test_result.txt')
            self.print_log(f"Saving result+wrong files to {rf}+{wf}")
...
if __name__ == '__main__':
    ...
    if arg.data_path is not None:
        arg.train_feeder_args["data_path"] = arg.data_path
        arg.test_feeder_args["data_path"]  = arg.data_path
        arg.val_feeder_args["data_path"]   = arg.data_path
        print(f"Using data path: {arg.data_path}")
